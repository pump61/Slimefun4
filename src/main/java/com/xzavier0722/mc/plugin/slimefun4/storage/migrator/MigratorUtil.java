...
class MigratorUtil {
    protected static boolean createDirBackup(File dir) {
        try {
            ...
            if (Files.exists(backupPath, LinkOption.NOFOLLOW_LINKS)) {
                Slimefun.logger().log(Level.WARNING, "Backup existente detectado, ignorando backup");
                return true;
            }
                    ...
                    fs.forEach(path -> {
                        ...
                        try {
                            ...
                            Slimefun.logger()
                                    .log(
                                            Level.WARNING,
                                            "Problema ao fazer backup dos dados antigos " + dir.getName(),
                                            e);
                        }
                    });
                }
            ...
            Slimefun.logger().log(Level.WARNING, "Problema ao fazer backup dos dados antigos " + dir.getName(), e);
            return false;
        }
    }
    ...
    protected static void deleteOldFolder(File dir) {
        try {
            ...
            Slimefun.logger().log(Level.WARNING, "Problema ao excluir pasta " + dir.getAbsolutePath(), e);
        }
    }
