    ...
    protected void addChannelSelector(Block b, BlockMenu menu, int slotPrev, int slotCurrent, int slotNext) {
        ...
        menu.replaceExistingItem(
                ...
                new CustomItemStack(
                        ...
                        "&bCanal Anterior",
                        "",
                        "&e> Clique para diminuir o ID do canal"));
        ...
        if (channel == 16) {
            menu.replaceExistingItem(
                    slotCurrent,
                    new CustomItemStack(
                            HeadTexture.CHEST_TERMINAL.getAsItemStack(), "&bID do Canal: &3" + (channel + 1)));
            menu.addMenuClickHandler(slotCurrent, ChestMenuUtils.getEmptyClickHandler());
        } else {
            menu.replaceExistingItem(
                    slotCurrent,
                    new CustomItemStack(ColoredMaterial.WOOL.get(channel), "&bID do Canal: &3" + (channel + 1)));
            menu.addMenuClickHandler(slotCurrent, ChestMenuUtils.getEmptyClickHandler());
        }
        ...
        menu.replaceExistingItem(
                ...
                new CustomItemStack(
                        ...
                        "&bPróximo Canal",
                        "",
                        "&e> Clique para aumentar o ID do canal"));
