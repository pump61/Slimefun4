...
public abstract class AbstractMonsterSpawner extends SlimefunItem implements DistinctiveItem {
    ...
    public Optional<EntityType> getEntityType(@Nonnull ItemStack item) {
        ...
        for (String line : meta.getLore()) {
            ...
            if ((stripColor.startsWith("Tipo: ") || stripColor.startsWith("Type:"))
                    && (!line.contains("<Tipo>") || line.contains("<Type>"))) {
    ...
    public ItemStack getItemForEntityType(@Nullable EntityType type) {
        ...
        for (int i = 0; i < lore.size(); i++) {
            ...
            if (currentLine.contains("<Type>") || currentLine.contains("<Tipo>")) {
                String typeName = type == null ? "Vazio" : ChatUtils.humanize(type.name());
                lore.set(i, currentLine.replace("<Type>", typeName).replace("<Tipo>", typeName));
                break;
            }
        }
