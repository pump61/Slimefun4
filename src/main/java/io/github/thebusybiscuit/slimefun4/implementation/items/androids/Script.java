...
public final class Script {
    ...
    ItemStack getAsItemStack(@Nonnull ProgrammableAndroid android, @Nonnull Player p) {
        ...
        lore.add("&7Avaliação: " + getScriptRatingPercentage());
        lore.add("&a" + getUpvotes() + " \u263A &7| &4\u2639 " + getDownvotes());
        lore.add("");
        lore.add("&eClique esquerdo &fBaixar script");
        lore.add("&4(Isso vai sobrescrever seu script atual!)");

        if (canRate(p)) {
            lore.add("");
            lore.add("&eShift + Clique esquerdo &fCurtir");
            lore.add("&eShift + Clique direito &fNão curtir");
        }
