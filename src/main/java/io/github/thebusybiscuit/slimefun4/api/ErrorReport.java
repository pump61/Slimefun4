...
public class ErrorReport<T extends Throwable> {
    ...
    public ErrorReport(T throwable, Location l, SlimefunItem item) {
        this(throwable, item.getAddon(), stream -> {
            stream.println("Informações do bloco:");
            stream.println("  Mundo: " + l.getWorld().getName());
            stream.println("  X: " + l.getBlockX());
            stream.println("  Y: " + l.getBlockY());
            stream.println("  Z: " + l.getBlockZ());
            stream.println("  Tipo do bloco: " + l.getBlock().getType());
            stream.println("  Dados do bloco: "
                    + l.getBlock().getBlockData().getClass().getName());
            stream.println("  Estado: " + l.getBlock().getState().getClass().getName());
            stream.println();

            if (item.getBlockTicker() != null) {
                stream.println("Informações do Ticker:");
                stream.println("  Tipo: " + (item.getBlockTicker().isSynchronized() ? "Síncrono" : "Assíncrono"));
                stream.println();
            }

            if (item instanceof EnergyNetProvider) {
                stream.println("Informações do Ticker:");
                stream.println("  Tipo: Indireto (gerenciado pela rede de energia)");
                stream.println();
            }

            stream.println("Dados Slimefun:");
                        ...
                        .ifPresentOrElse(
                                uuid -> {
                                    ...
                                    if (universalData != null) {
                                        stream.println(
                                                "  Status de carregamento de dados: " + universalData.isDataLoaded());
                                        stream.println("  Inventário: " + (universalData.getMenu() != null));
                                        stream.println("  Dados: ");
                                        universalData
                                                .getAllData()
                                                .forEach((k, v) -> stream.println("    " + k + ": " + v));
                                    } else {
                                        stream.println("Este bloco não possui dados.");
                                    }
                                },
                                () -> stream.println("Este bloco não possui dados.")));
            } else {
                stream.println("  Status de carregamento de dados: " + blockData.isDataLoaded());
                stream.println("  Inventário: " + (blockData.getBlockMenu() != null));
                stream.println("  Dados: ");
