    ...
    public static void renderPageButton(
        ...
        if (totalPages > 1) {
            if (page > 1) {
                menu.addItem(
                        prevSlot,
                        new CustomItemStack(Material.ARROW, "&aAnterior &7(" + page + "/" + totalPages + ")"));
                menu.addMenuClickHandler(prevSlot, prevHander);
            } else {
                menu.addItem(prevSlot, new CustomItemStack(Material.GRAY_STAINED_GLASS_PANE, "&7Anterior (nenhuma)"));
                menu.addMenuClickHandler(prevSlot, ChestMenuUtils.getEmptyClickHandler());
            }

            if (page < totalPages) {
                menu.addItem(
                        nextSlot, new CustomItemStack(Material.ARROW, "&aPróxima &7(" + page + "/" + totalPages + ")"));
                menu.addMenuClickHandler(nextSlot, nextHander);
            } else {
                menu.addItem(nextSlot, new CustomItemStack(Material.GRAY_STAINED_GLASS_PANE, "&7Próxima (nenhuma)"));
