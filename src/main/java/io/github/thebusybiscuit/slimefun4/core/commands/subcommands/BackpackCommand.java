...
class BackpackCommand extends SubCommand {
                ...
                if (args.length < 1) {
                    Slimefun.getLocalization()
                            .sendMessage(
                                    ...
                                    "messages.usage",
                                    true,
                                    msg -> msg.replace("%usage%", "/sf backpack (nome do jogador)"));
                    return;
                }
    ...
    private void showBackpackMenu(OfflinePlayer owner, Player p, Set<PlayerBackpack> result, int page) {
        var menu = new ChestMenu("Mochilas de " + owner.getName());
        ...
        for (int i = 0; i <= 36; i++) {
            ...
            im.setDisplayName(bp.getName().isEmpty() ? "Mochila #" + bp.getId() : bp.getName());
            var lore = new ArrayList<String>();
            lore.add("");
            lore.add(ChatColor.translateAlternateColorCodes('&', "&aClique para obter esta mochila"));
            im.setLore(lore);
