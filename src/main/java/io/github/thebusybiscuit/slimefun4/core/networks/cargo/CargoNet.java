...
public class CargoNet extends AbstractItemNetwork implements HologramOwner {
    ...
    public void tick(@Nonnull Block b, SlimefunBlockData blockData) {
        if (!regulator.equals(b.getLocation())) {
            updateHologram(b, "&4Múltiplos reguladores de rede de carga detectados", blockData::isPendingRemove);
            return;
        }
        ...
        if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
            updateHologram(b, "&cNó de rede de carga não encontrado", blockData::isPendingRemove);
        } else {
            updateHologram(b, "&7Status: &a&lConectado", blockData::isPendingRemove);
